        ...
        for (int j = 0; j < 36; j++) {
            ...
            for (int k = 0; k < numTriangles; k++) {
                ...
                if (!isnan(nearestIntersection.x)) {
                    char redisKey[128];
                    snprintf(redisKey, sizeof(redisKey), "intersection:%d", intersectionIndex++);
                    ...
                    reply = redisCommand(conn, "HMSET %s observed_point_id %d intersection_id %d ray_angle %d nearest_intersection_x %.2f nearest_intersection_y %.2f nearest_intersection_z %.2f",
                                        redisKey, i + 1, intersectionIndex - 1, ray_angle, nearestIntersection.x, nearestIntersection.y, nearestIntersection.z);
