...
int main() {
    ...
    if (availableMemory <= (long long)requiredMemorySize) {
        ...
        if (rank == 0){
            ...
            printf("Rank %d: broadcast points, %f s \n", rank, MPI_Wtime() - start_time);
            ...
            MPI_Bcast(localPoints, numLocalPoints * sizeof(Point3D), MPI_BYTE, 0, MPI_COMM_WORLD);
            printf("Rank %d: receive bcast_points, %f s \n", rank, MPI_Wtime() - start_time);
        }
        ...
        for (int batch = 0; batch < numBatches; batch++) {
            ...
            if (rank == 0) {
                ...
                if (successTriangle) {
                    ...
                    printf("Rank %d: broadcast triangles, %f s \n", rank, MPI_Wtime() - start_time);
                ...
                if (successTriangle) {
                    ...
                    printf("Rank %d: receive bcast_triangles, %f s \n\n", rank, MPI_Wtime() - start_time);
                } else {
            ...
            allBatchesResults[batch] = checkLineIntersections(localPoints, numLocalPoints, localTriangles, numTrianglesThisBatch, &totalResultsPerBatch[batch]);        
            printf("batch: %d, Rank %d: finish calculation, %f s \n", batch, rank, MPI_Wtime() - start_time);
            MPI_Barrier(MPI_COMM_WORLD);
        ...
        if (rank == 0) {
            ...
            int filteredCount;
            filterResults(allCombinedResults, totalAllBatchesResults, &filteredResults, &filteredCount);

            // 将 filteredResults 结果写入文件
            FILE *filteredFile = fopen("filtered_intersections.csv", "w");
            if (filteredFile != NULL) {
                fprintf(filteredFile, "PointID,DirectionID,IntersectionX,IntersectionY,IntersectionZ,Length\n");
                for (int i = 0; i < filteredCount; i++) {
                    fprintf(filteredFile, "%d,%d,%.2f,%.2f,%.2f,%.2f\n",
                        filteredResults[i].pointId,
                        filteredResults[i].directionId,
                        filteredResults[i].intersectionPoint.x,
                        filteredResults[i].intersectionPoint.y,
                        filteredResults[i].intersectionPoint.z,
                        filteredResults[i].directionLength);
                }
                fclose(filteredFile);
            }
