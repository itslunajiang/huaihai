...
void checkLineIntersections(Point3D *points, int numPoints, Triangle3D *triangles, int numTriangles) {
    ...
    if (rank == 0) {
        ...
        for (int i = 0; i < totalResults; i++) {
            ...
            // 创建哈希表
            reply = (redisReply *)redisCommand(conn, "HMSET %s observed_point_id %d intersection_id %d ray_angle %d nearest_intersection_x %.2f nearest_intersection_y %.2f nearest_intersection_z %.2f",
...
DWORDLONG getAvailableMemory() {
    ...
    if (GlobalMemoryStatusEx(&memoryStatus)) {
        return (DWORDLONG)memoryStatus.ullAvailPhys / (1024 * 1024); // 可用物理内存,单位MB
    } else {
        fprintf(stderr, "无法获取可用内存: %d\n", (int)GetLastError());
        return 1;
    }
}
