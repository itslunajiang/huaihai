...
void checkLineIntersections(Point3D *points, int numPoints, Triangle3D *triangles, int numTriangles) {
    ...
    int trianglesPerProcess = numTriangles / size;
    int remainder = numTriangles % size;

    int startTriangle = rank * trianglesPerProcess + (rank < remainder ? rank : remainder);
    int endTriangle = startTriangle + trianglesPerProcess + (rank < remainder ? 1 : 0);
    ...
    for (int i = 0; i < numPoints; i++) {
       for (int offset = 1; offset <= 5; offset++){
        for (int j = 0; j < 36; j++) {
            ...
            // 遍历所有三角面
            for (int k = startTriangle; k < endTriangle; k++) {
    ...
    if (rank == 0) {
        ...
        for (int i = 0; i < totalResults; i++) {
            char redisKey[128];
            snprintf(redisKey, sizeof(redisKey), "intersection:%d", i + 1);

            // 创建哈希表
            reply = redisCommand(conn, "HMSET %s observed_point_id %d intersection_id %d ray_angle %d nearest_intersection_x %.2f nearest_intersection_y %.2f nearest_intersection_z %.2f",
                                redisKey, allResults[i].pointId, allResults[i].intersectionId, allResults[i].rayAngle, allResults[i].intersectionPoint.x, allResults[i].intersectionPoint.y, allResults[i].intersectionPoint.z);
...
int64_t getAvailableMemory() {
    ...
    int64_t free_memory = (int64_t)vm_stat.free_count * (int64_t)page_size;
    return free_memory / (1024 * 1024);
}


// 计算 double 数组中所有元素所需的内存大小（单位：MB）
...
int main() {
    ...
    size_t requiredMemorySize = pointDataSize + trianglesDataSize;


    // 根进程打印 requiredMemorySize 和 availableMemory 的值
    if (rank == 0) {
        // 打印 requiredMemorySize 和 availableMemory 的值
        printf("requiredMemorySize: %zu MB\n", requiredMemorySize);
        printf("availableMemory: %lld MB\n", availableMemory);
    }
    ...
    // 列出所有哈希表键的名称
    //redisReply *reply = redisCommand(conn, "KEYS intersection:*");
    //if (reply == NULL || reply->type != REDIS_REPLY_ARRAY) {
        //printf("Error listing keys\n");
        //exit(1);
    //}
    ...
    //for (size_t i = 0; i < reply->elements; i++) {
        ...
        //if (keyReply->type == REDIS_REPLY_STRING) {
            // 获取哈希表内容
            //redisReply *hgetallReply = redisCommand(conn, "HGETALL %s", keyReply->str);
            //if (hgetallReply == NULL || hgetallReply->type != REDIS_REPLY_ARRAY) {
                //printf("Error getting hash data\n");
            //} else {
                //for (size_t j = 0; j < hgetallReply->elements; j += 2) {
                    //printf("%s: %s\n", hgetallReply->element[j]->str, hgetallReply->element[j + 1]->str);
                //}
            //}
    ...
    if (rank == 0) {
        ...
        printf("程序运行期间的最高峰内存使用情况: %ld MB\n", peakMemoryDelta);
        printf("程序运行前后的内存差值: %ld MB\n", differenceMemory);
    }
